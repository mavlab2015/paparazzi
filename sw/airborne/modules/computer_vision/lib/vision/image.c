...
struct line_deviation_t line_follow(struct image_t *input, struct image_t *output, uint8_t w, uint8_t th)
{
  ...
  uint8_t *dest = output->buf;
  uint16_t x, y, i, j;
  uint8_t boundary1[240], integral1[240] = {};
  uint8_t boundary2[320], integral2[320] = {};
  uint8_t boundary3[240], integral3[240] = {};
  uint16_t integral1_max, integral2_max, integral3_max, integral_max_max = 0;
  uint16_t idx1, idx2, idx3 = 0;
  uint16_t sum1, sum2, sum3 = 0;
  uint16_t max_idx1[2][1], max_idx2[2][1], max_idx3[2][1], max_idx[2][1] = {};
  ...
  for (i= 0; i < input->h - w; i++)
  {
  	boundary1[i] = image[i][0];
  	boundary3[i] = image[i][input->w];
  	sum1 = 0;
  	sum3 = 0;
  	...
  	if (integral3[idx3] > integral3_max)
  	{
  		...
  		max_idx3[0][0] = i;
  		max_idx3[1][0] = input->w;
  	}
  	idx3 += 1;
  	
  }
  
  for (i= 0; i < input->w - w; i++)
  {
  	boundary2[i] = image[0][i];
  	sum2 = 0;
  	for (j = 0; j < w+1; j++)
  	{
  		sum2 += image[0][i+j];
  ...
  if (integral1_max > integral_max)
  	integral_max = integral1_max;
  if (integral2_max > integral_max)
  	integral_max = integral2_max;
  if (integral3_max > integral_max)
  	integral_max = integral3_max;
  	
  if (integral_max == integral2_max)
  {
  	max_idx[0][0] = max_idx2[0][0];
  	max_idx[1][0] = max_idx2[0][0];
  }
  else
  {
  	...
  	if (integral1_max < w*th && integral3_max < w*th)
  	{
  		max_idx[0][0] = input->h / 2;
  		max_idx[1][0] = input->w / 2;
  	}
  	if (integral1_max > w*th && integral3_max > w*th)
  	{
